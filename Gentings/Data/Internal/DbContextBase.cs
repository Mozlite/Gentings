...
namespace Gentings.Data.Internal
{
    ...
    public abstract class DbContextBase<TModel> : IDbContextBase<TModel>
    {
        ...
        public virtual TQuery Load<TQuery>(TQuery query, Expression<Func<TModel, object>> countExpression = null)
        ...
        /// <param name="countExpression">返回总记录数的表达式,用于多表拼接过滤重复记录数。</param>
        /// <returns>返回分页实例列表。</returns>
        public virtual TQuery Load<TQuery, TObject>(TQuery query, Expression<Func<TModel, object>> countExpression = null) where TQuery : QueryBase<TModel, TObject>
        {
            var context = AsQueryable();
            query.Init(context);
            query.Models = context.AsEnumerable<TObject>(query.Page, query.PageSize, countExpression);
            return query;
        }
        ...
        /// <returns>返回分页实例列表。</returns>
        public virtual Task<TQuery> LoadAsync<TQuery>(TQuery query,
            ...
            => LoadAsync<TQuery, TModel>(query, countExpression, cancellationToken);
        ...
        /// <param name="cancellationToken">取消标识。</param>
        /// <returns>返回分页实例列表。</returns>
        public virtual async Task<TQuery> LoadAsync<TQuery, TObject>(TQuery query, Expression<Func<TModel, object>> countExpression = null,
            CancellationToken cancellationToken = default) where TQuery : QueryBase<TModel, TObject>
        {
            var context = AsQueryable();
            query.Init(context);
            query.Models = await context.AsEnumerableAsync<TObject>(query.Page, query.PageSize, countExpression, cancellationToken);
            return query;
        }
